...
/** A predicted physical quantity with stated confidence. This is the pitch. */
...
/**
 * Physics metadata stored on `assets.physics` (JSONB). Every field is a value
 * the pipeline predicted. Kept open-ended so new quantities can be added
 * without a migration; the gallery renders `mass`, `friction`, and `inertia`
 * up front.
 */
